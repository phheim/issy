...
-- | Create a priority queue with a single priority from a list
fromList :: p -> [a] -> PrioQueue p a
fromList p xs
  | null xs = empty
  | otherwise = PrioQueue $ Map.singleton p $ Q.fromList xs

-- | Return the first element according to the queuing order, its priority, and the
...
-- | Append an element with a given priority to the queue.
...
-- | Append a list of elements with a single priority to the queue.
